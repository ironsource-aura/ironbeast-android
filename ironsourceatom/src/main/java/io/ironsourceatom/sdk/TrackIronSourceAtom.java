...
public class TrackIronSourceAtom {
     ...
     */
    public TrackIronSourceAtom(Context context) {
        mContext = context;
        mConfig = IBConfig.getInstance(context);
    }

    public static TrackIronSourceAtom getInstance() {return  sInstance;}

    public static TrackIronSourceAtom getInstance(Context context) {
        if (null == context) {
            throw new IllegalArgumentException("`context` should be valid Context object");
        }
        synchronized (sInstanceLockObject) {
            if (sInstance == null) {
                sInstance = new TrackIronSourceAtom(context.getApplicationContext());
            }
        }
        return sInstance;
    }
